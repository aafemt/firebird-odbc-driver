...
static Types types [] =
{
	BLOB ("BLOB SUB_TYPE TEXT", JDBC_WLONGVARCHAR, MAX_WLONGVARCHAR_LENGTH, "'","'",CASE_SENSITIVE),		// 0
	ALPHA ("VARCHAR", JDBC_WVARCHAR, MAX_WVARCHAR_LENGTH),													// 1
	ALPHA ("CHAR", JDBC_WCHAR, MAX_WCHAR_LENGTH),															// 2
	NUMERIC ("BOOLEAN", JDBC_BOOLEAN, MAX_BOOLEAN_LENGTH, "", UNSCALED, UNSCALED, NOT_NUMERIC),				// 3
	NUMERIC ("SMALLINT", JDBC_TINYINT, MAX_SMALLINT_LENGTH, "", 0, 0, 10),									// 4
	NUMERIC ("BIGINT", -5, MAX_QUAD_LENGTH,"", 0, MAX_QUAD_LENGTH, 10),										// 5
	BLOB ("BLOB SUB_TYPE 0", JDBC_LONGVARBINARY, MAX_BLOB_LENGTH, "","",CASE_INSENSITIVE),					// 6
	BLOB ("VARBINARY", JDBC_VARBINARY, MAX_BLOB_LENGTH, "","",CASE_INSENSITIVE),							// 7
	BLOB ("BINARY", JDBC_BINARY, MAX_BLOB_LENGTH, "","",CASE_INSENSITIVE),									// 8
	BLOB ("BLOB SUB_TYPE TEXT", JDBC_LONGVARCHAR, MAX_BLOB_LENGTH, "'","'",CASE_SENSITIVE),					// 9
	ALPHA ("CHAR", JDBC_CHAR, MAX_CHAR_LENGTH),																// 10
	NUMERIC ("NUMERIC", JDBC_NUMERIC, MAX_NUMERIC_LENGTH, "precision,scale", 0, MAX_NUMERIC_LENGTH, 10),	// 12
	NUMERIC ("DECIMAL", JDBC_DECIMAL, MAX_DECIMAL_LENGTH, "precision,scale", 0, MAX_DECIMAL_LENGTH, 10),	// 13
	NUMERIC ("INTEGER", JDBC_INTEGER, MAX_INT_LENGTH, "", 0, 0, 10),										// 14
	NUMERIC ("SMALLINT", JDBC_SMALLINT, MAX_SMALLINT_LENGTH, "", 0, 0, 10),									// 15
	NUMERIC ("DOUBLE PRECISION", JDBC_FLOAT, MAX_DOUBLE_DIGIT_LENGTH, "", UNSCALED, UNSCALED, 2),			// 16
	NUMERIC ("FLOAT", JDBC_REAL, MAX_FLOAT_DIGIT_LENGTH, "", UNSCALED, UNSCALED, 2),						// 17
	NUMERIC ("DOUBLE PRECISION", JDBC_DOUBLE, MAX_DOUBLE_DIGIT_LENGTH, "", UNSCALED, UNSCALED, 2),			// 18
	DATE("DATE",9,MAX_TIMESTAMP_LENGTH,"date'","'",1),														// 19
	DATETIME("TIME",9,MAX_TIME_LENGTH,"time'","'",2),														// 20
	DATETIME("TIMESTAMP",9,MAX_TIMESTAMP_LENGTH,"timestamp'","'",3),										// 21
	ALPHA ("VARCHAR", JDBC_VARCHAR, MAX_VARCHAR_LENGTH),													// 22
	DATE("DATE",JDBC_DATE,MAX_DATE_LENGTH,"{d'","'}",1),													// 23
	DATETIME("TIME",JDBC_TIME,MAX_TIME_LENGTH,"{t'","'}",2),												// 24
	DATETIME("TIMESTAMP",JDBC_TIMESTAMP,MAX_TIMESTAMP_LENGTH,"{ts'","'}",3)									// 25
};
...
TypesResultSet::TypesResultSet(int dataType, int appOdbcVersion, int bytesPerCharacter) : IscResultSet (NULL)
{
	...
	types[0].typePrecision = MAX_BLOB_LENGTH / bytesPerCharacter;
	types[1].typePrecision = MAX_VARCHAR_LENGTH / bytesPerCharacter;
	types[2].typePrecision = MAX_CHAR_LENGTH / bytesPerCharacter;
	types[9].typePrecision = MAX_BLOB_LENGTH / bytesPerCharacter;
	types[10].typePrecision = MAX_CHAR_LENGTH / bytesPerCharacter;
	types[22].typePrecision = MAX_VARCHAR_LENGTH / bytesPerCharacter;
	...
	SET_SQLVAR( 4, "LITERAL_PREFIX"		, SQL_VARYING	,	13	, OFFSET(Types,lenTypePrefix)			)
	SET_SQLVAR( 5, "LITERAL_SUFFIX"		, SQL_VARYING	,	11	, OFFSET(Types,lenTypeSuffix)			)
